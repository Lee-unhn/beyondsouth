...
interface Speaker {
  ...
  session?: string;
  photo?: string;
  quote?: string;        // 核心金句 (Google Sheet column: quote)
  instagram?: string;   // 社群連結 (Google Sheet column: instagram)
  linkedin?: string;    // 社群連結 (Google Sheet column: linkedin)
  twitter?: string;     // 社群連結 (Google Sheet column: twitter)
  facebook?: string;    // 社群連結 (Google Sheet column: facebook)
}
      ...
      <section id="hero" className="min-h-screen flex flex-col justify-center items-center text-center px-6 pt-32 pb-20 relative">
        ...
        >
          ...
          <button
            ...
            className="text-gray-light border-b border-white/20 py-3.5 md:py-4 text-xs md:sm tracking-widest hover:text-teal hover:border-teal flex items-center gap-2 transition-all"
          >
              ...
              speakers.map((s, i) => (
                ...
                >
                  <div className="h-[210px] relative overflow-hidden bg-[#0f0f28] flex items-center justify-center">
                    {s.photo ? (
                      <img src={s.photo} alt={s.name} className="w-full h-full object-contain transition-transform duration-500 group-hover:scale-105" />
                    ) : (
                  ...
                  <div className="p-5 flex flex-col gap-1.5">
                    ...
                    <div className="text-xs text-teal">{s.title}</div>
                    <div className="text-[10px] text-gray-muted/60">{s.company}</div>
                    {s.quote && (
                      <div className="text-[11px] italic text-white/55 border-t border-white/5 pt-2.5 mt-1 leading-relaxed">
                        &#x201C;{s.quote}&#x201D;
                      </div>
                    )}
                    {(s.instagram || s.linkedin || s.twitter || s.facebook) && (
                      <div className="flex flex-wrap gap-1.5 border-t border-white/5 pt-2.5 mt-1">
                        {s.instagram && <a href={s.instagram} target="_blank" rel="noreferrer" className="font-mono text-[9px] text-teal border border-teal/25 px-2 py-0.5 hover:bg-teal/10 transition-colors tracking-wider">IG</a>}
                        {s.linkedin && <a href={s.linkedin} target="_blank" rel="noreferrer" className="font-mono text-[9px] text-purple border border-purple/25 px-2 py-0.5 hover:bg-purple/10 transition-colors tracking-wider">IN</a>}
                        {s.twitter && <a href={s.twitter} target="_blank" rel="noreferrer" className="font-mono text-[9px] text-teal border border-teal/25 px-2 py-0.5 hover:bg-teal/10 transition-colors tracking-wider">TW</a>}
                        {s.facebook && <a href={s.facebook} target="_blank" rel="noreferrer" className="font-mono text-[9px] text-purple border border-purple/25 px-2 py-0.5 hover:bg-purple/10 transition-colors tracking-wider">FB</a>}
                      </div>
                    )}
                  </div>
                </motion.div>
          ...
          <div className="flex flex-col gap-0.5">
            {[
              ...
              <div key={i} className={cn(
                ...
                {/* Time block */}
                <div style={{ minWidth: '88px' }} className="flex flex-col justify-center items-center py-6 px-4 border-r border-white/5">
                  <span style={{
                    fontFamily: 'ui-monospace, monospace',
                    fontSize: '14px',
                    fontWeight: '700',
                    color: (row.style === 'adm' || row.style === 'brk') ? 'rgba(255,255,255,0.18)' : '#00C9B1'
                  }}>
                    {row.t}
                  </span>
                </div>
                {/* Content block */}
                <div className="flex-1 px-6 py-5 flex flex-col justify-center" style={{ gap: '8px' }}>
                  <div>
                    <span className={cn(
                      "inline-block px-2.5 py-0.5 font-mono font-bold tracking-widest",
                  ...
                  <div style={{
                    color: (row.style === 'adm' || row.style === 'brk') ? 'rgba(255,255,255,0.28)' : '#fff',
                    fontWeight: row.style === 'talk' ? '700' : '400',
                    fontSize: row.style === 'talk' ? '15px' : '14px',
                    lineHeight: '1.4'
                  }}>
                    {row.title}
                  </div>
